...
#pragma omp parellel for  private(k) reduction(+,asum) reduction(+,fsum) schedule(static,1000)
    for ( k = 0; k < nvecs; ++k ) {
        asum += alpha[k];
        fsum += alpha[k] * grad[k] * dy[k];
    }
    fsum = ( fsum + asum ) *0.5;
#pragma omp parellel for  private(k) reduction(+,bias) reduction(+,nfree)  schedule(static,1000)
    for ( k = 0; k < nvecs; ++k ) {
        if ( smo->status[k] != 0 )
            continue;
        bsum += grad[k] ;
...
///// if nfree = 0 then bias = 0 so no need for an else here
    if ( nfree )
        bsum /= nfree;
#pragma omp parellel for  private(k) reduction(+,csum)  schedule(static,1000)
    for ( k = 0; k < nvecs; ++k ) {
...
#pragma omp parellel for  private(k)  schedule(static,1000)
            for ( k = 0; k < nvecs; ++k ) {
...
void IndexMax( IndexPair *p1, IndexPair *p2 ) {
...
void IndexMin( IndexPair *p1, IndexPair *p2 ) {
    if (p1->value <= p2->value) return;
    p1->value = p2->value;
    p1->index = p2->index;
    return;
...
#endif    
#pragma omp parallel private(my_max,my_min,tid) shared(nth,max_pair,min_pair,the_max,the_min)
    {
        ...
        my_min.value = max0;
        my_min.index = -1;
#pragma omp parallel for private(k,gx,ys) schedule(static,1000)
        for (k=0;k<nvecs;++k) {
...
#pragma omp parallel num_threads(2) shared(max_pair,min_pair,the_max,the_min,i,nth) 
    {
#pragma single
      {
