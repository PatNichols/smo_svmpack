...
namespace svmpack {
void smo_solver_train(const svm_options& options)
{
  try {
    ...
    stopwatch timer;
    timer.clear();
    std::string data_file = options.data;
    svmpack::svm_data data;
    data.read_data_file(data_file);
    svmpack::smo_solver smo(options,data);
...
#endif
    smo.output_model_file(options,data);
  }
...
#endif
    const double *qmax = kmatrix.getRow(imax);
    const double *qmin = kmatrix.getRow(imin);
#ifdef SVMPACK_USE_TIMERS
    kmat_timer.stop();
    step_timer.start();
#endif
...
void smo_solver::output_model_file(const svm_options& opts,
                                    const svm_data& data) {
    const int nfeat = data.num_features();
    const std::string modelfile = opts.model;
    const std::string outfile = opts.out;
    int nsv = 0;
    int nbnd = 0;
    int ntp = 0 ;
    int nfp = 0 ;
    ...
    double dtmp;
    double fx;
    const double *scal = kmatrix.scaleFactors();
    double *v;
#pragma omp parallel for  private(k) reduction(+:nsv) reduction(+:nbnd)
    ...
    if (!out) {
        std::cerr << "could not open model file " << modelfile << " for output\n";
        exit(-1);
    }
    out.write((const char*)&nsv,sizeof(int));
    out.write((const char*)&(nfeat),sizeof(int));
    out.write((const char*)&(opts.ktype),sizeof(int));
    out.write((const char*)&(opts.kpow),sizeof(int));
    out.write((const char*)&(opts.kc1),sizeof(double));
    out.write((const char*)&(opts.kc2),sizeof(double));
    out.write((const char*)&(bias),sizeof(double));
    int itmp = opts.kscale ? 1:0;
    out.write((const char*)&(itmp),sizeof(int));
    for ( k = 0; k < nvecs; ++k ) {
        if ( status[k] < 0 ) continue;
        dtmp = y[k] * scal[k] * alfa[k];
        out.write((const char*)&dtmp,sizeof(double));
    }
    const double * vf = data.vectors();
    for ( k = 0; k < nvecs ; ++k) {
        if (status[k]<0) continue;
        const double * v = vf + k * nfeat;
        out.write((const char*)v,nfeat*sizeof(double));
    }
    out.close();
    fprintf(stderr,"wrote model file\n");
    std::ofstream fout(outfile.c_str());
    if (!fout) {
        std::cerr << "could not open output file " << outfile << " for output\n";
        exit(-1);
    }    
    for ( k = 0; k < nvecs; ++k ) {
        fx = ( y[k] - grad[k] - bias );
        ...
        dtmp = y[k];
        fout.write((const char*)&dtmp,sizeof(double));
        fout.write((const char*)&fx,sizeof(double));
    }
    fout.close();
    std::cout << "trainer self validation\n";
